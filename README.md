...
Generated by [Infracost](https://www.infracost.io/) from [`infracost.yml`](infracost.yml) (one
project per module, using the same representative values as
`terragrunt/live/shared/*/terragrunt.hcl`), kept up to date by the `Cost` CI job — see
[CI/CD](#cicd). Static analysis, no AWS credentials or deployed infrastructure involved, so it's
...
- **Cost** — [Infracost](https://www.infracost.io/) breakdown from [`infracost.yml`](infracost.yml)
  (static analysis per module, no AWS credentials needed — same reasoning as `Test`). Needs a
  free account: sign up at infracost.io, then set repo secret `INFRACOST_API_KEY` to your API
  key (Org Settings → API Keys) — without it this job fails outright (no silent skip, unlike the
  old `Build` job). Not a required status check. Same self-commit rule as `Diagram` below: only
  on push to `develop`, updating the [Cost estimate](#cost-estimate) section of this README.
